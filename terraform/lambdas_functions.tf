...
resource "aws_lambda_function" "notify_library_book_registered" {
  ...
  environment {
    variables = {
      WEBSOCKET_URL = "https://${aws_apigatewayv2_api.websocket_api.id}.execute-api.${data.aws_region.current.region}.amazonaws.com/${aws_apigatewayv2_stage.websocket_stage.name}"
    }
  }
